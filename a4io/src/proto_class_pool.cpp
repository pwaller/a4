...
shared<google::protobuf::Message> ProtoClassPool::get_new_message(
    uint32_t class_id) {
...
shared<google::protobuf::Message> ProtoClassPool::get_new_message(
    ...
    return shared<google::protobuf::Message>(_message_factory->GetPrototype(
                                                 d)->New());
}

shared<google::protobuf::Message> ProtoClassPool::parse_message(
    uint32_t                                       class_id,
...
#if 0
shared<A4Message> ProtoClassPool::read(
    uint32_t                                 class_id,
...
void ProtoClassPool::add_protoclass(const ProtoClass & protoclass) {
    ...
    const Descriptor * gd = _descriptor_pool->FindMessageTypeByName(
        protoclass.full_name());
    if (!gd) {
    ...
    if (reg.descriptor) {
        std::string cmajor = reg.descriptor->options().GetExtension(
            major_version);
        std::string cminor = reg.descriptor->options().GetExtension(
            minor_version);
        std::string dmajor = gd->options().GetExtension(major_version);
        std::string dminor = gd->options().GetExtension(minor_version);
        if (cmajor != dmajor) {
            FATAL(
                "Major versions of compiled-in and read messages do not agree:",
                "Compiled in: '",
                cmajor,
                "', Read: '",
                dmajor,
        ...
        _class_id_reader[class_id]     = bind(&ProtoClassPool::new_protoclass,
                                              this,
                                              prototype);
