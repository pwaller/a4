...
internal::classreg internal::map_class(std::string        name,
                                       uint32_t           lookup_class_id,
                                       internal::classreg reg,
                                       bool               warn) {
    ...
    if (reg.descriptor) {
        ...
        foreach (auto & i, all_classes) {
            ...
            if (i.second.descriptor->options().HasExtension(a4::io::
                                                            fixed_class_id)) {
                uint32_t class_id = i.second.descriptor->options().GetExtension(
                    a4::io::fixed_class_id);
