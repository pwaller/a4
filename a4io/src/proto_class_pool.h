...
public:
    ...
    shared<google::protobuf::Message> get_new_message(
        const google::protobuf::Descriptor * d) const;

    shared<google::protobuf::Message> parse_message(
        uint32_t                                       class_id,
    ...
    bool check_match(uint32_t                             class_id,
                     const google::protobuf::Descriptor * d) {
        if ((class_id < _class_id_descriptor.size()) &&
            (_class_id_descriptor[class_id] == d)) { return true; }
        return false;
    }
