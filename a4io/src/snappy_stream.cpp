...
bool SnappyInputStream::Next(const void ** data, int * size) {
    ...
    success = snappy::RawUncompress(
        tempbuffer.get(), compressed_size, _output_buffer.get());
    assert(success);
...
bool SnappyOutputStream::Flush() {
    ...
    shared<char> compressed_data(new char[snappy::MaxCompressedLength(
                                              size)], array_delete<char>());
    snappy::RawCompress(_input_buffer.get(), size,
