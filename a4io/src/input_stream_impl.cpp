    ...
    if (!_discovery_complete) {
        if (!_current_metadata_refers_forward) {
            ...
            if (_metadata_per_header[_current_header_index].size() > 0) {
                _current_metadata =
                    _metadata_per_header[_current_header_index][0];
            }
        } else {
            _current_metadata_index = -1;
            if (discovery_requested and not discover_all_metadata()) {
                ERROR(
                    "a4::io:InputStreamImpl - Failed to discover metadata - file corrupted?");
                return set_error();
            }
        }
    }
...
bool InputStreamImpl::discover_all_metadata() {
    ...
    while (true) {
        ...
        foreach (uint64_t offset, footer->protoclass_offsets()) {
            uint64_t metadata_start = footer_abs_start - footer->size() +
                                      offset;
        ...
        foreach (auto & cc, * _footers.back().mutable_class_count()) {
            cc.set_class_name(_current_class_pool->descriptor(
                                  cc.class_id())->full_name());
        }
        ...
        _temp_metadata_offset_per_header.push_front(
            _this_headers_metadata_offsets);
    ...
    _headers_forward.insert(_headers_forward.end(),
                            _temp_headers_forward.begin(),
                            _temp_headers_forward.end());
    return true;
}
...
bool InputStreamImpl::carry_metadata(uint32_t & header, int32_t & metadata) {
    ...
    while (header<_metadata_offset_per_header.size()
                  and metadata> static_cast<int32_t>(_metadata_offset_per_header
                                                     [header].size())) {
        metadata -= _metadata_offset_per_header[header].size();
        header   += 1;
    }
...
bool InputStreamImpl::seek_to(uint32_t header, int32_t metadata, bool carry) {
    ...
    if (!_discovery_complete) {
        if (seek(0) == -1) {
            ERROR(
                "a4::io:InputStreamImpl - Cannot skip in this unseekable stream!");
            set_error();
            return false;
        }
    ...
    if (carry and not carry_metadata(header, metadata)) { // modifies header and metadata
        if (not carry) {
            ERROR(
                "a4::io:InputStreamImpl - Attempt to seek to nonexistent metadata!");
        }
        return false;
    }
    ...
    if (_headers_forward[header]) {
        ...
        if (metadata ==
            static_cast<int32_t>(_metadata_offset_per_header[header].size())) {
        ...
        if ((metadata == 0) && (header == 0)) {
            ...
            if (metadata ==
                static_cast<int32_t>(_metadata_offset_per_header[header].
                                     size())) {
                // No more metadata in this header, current position is end
                // of stream.
                set_end();
...
bool InputStreamImpl::start_compression(const StartCompressedSection & cs) {
    ...
    if (cs.compression() == StartCompressedSection_Compression_ZLIB) {
        _compressed_in.reset(new GzipInputStream(_raw_in.get(),
                                                 GzipInputStream::ZLIB));
