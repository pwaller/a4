...
public:
    ...
    void print(std::ostream & out, const T & value, const Args & ... args) {
        if (measuring) {
            ...
            out << std::setw(lengths.at(current_index)) << std::fixed <<
                value << " ";
        }
...
public:
    ...
    void collect(const double & value) {
        ...
        if (value <= min) { min = value; } else if (value < min1) {
            min1 = value;
        }

        if (value >= max) { max = value; } else if (value > max1) {
            max1 = value;
        }
...
#undef HANDLE_TYPE
                    ...
                    case FieldDescriptor::CPPTYPE_MESSAGE:
                        collect(reflection->GetRepeatedMessage(message,
                                                               field,
                                                               j));
                        break;
...
public:
    ...
    size_t collect(const Message & message) {
        ...
        stats[desc->full_name() + ":fields"].collect(
            fields.size() - sub_fields, 0);
        return this_size;
    }
    ...
    friend std::ostream & operator<<(std::ostream               & o,
                                     MessageInfoCollector const & mic) {
...
public:
    ...
    Selection(const std::string & sel) :
        _desc(NULL),
        _field_desc(NULL) {
        auto pos = sel.find(":");
...
public:
    ...
    CheckSelection(const Message & m) :
        _m(m) {}
...
bool dump_stream(shared<a4::io::InputStream>    stream,
    ...
    if (dump_proto) {
        ...
        foreach (const auto * file_descriptor, file_descriptors) {
            ...
            auto * evm =
                file_descriptor->FindMessageTypeByName("EventMetaData");
            if (evm) {
                DEBUG("Here: ", evm->name());
    ...
    for (; dump_all || event_count; event_count--) {
        shared<A4Message> m =
            internal_msg ? stream->next_bare_message() : stream->next();
        if (!m) {
            break;
        }
...
int main(int argc, char ** argv) {
    ...
    commandline_options.add_options()
        ...
        ("event-index,i", po::value(&event_index),
        "event to start dumping from (starts at 0)")
        ("all,a", po::bool_switch(&dump_all), "dump all events")
        ("number,n", po::value(&event_count), "maximum number to dump")
        ("internal,I", po::bool_switch(&internal_msg),
        "also dump stream internal messages")
        ("collect-stats,S", po::bool_switch(&collect_stats),
        "should collect statistics for all numeric variables")
        ("message-info,M", po::bool_switch(&message_info),
        "should collect statistics relating to the message")
        ("short-form,s", po::bool_switch(&short_form),
        "print in a compact form, one event per line")
        ("select", po::value(&selection_strings),
        "Select messages by string equality (e.g. --select event_number:1234)")
        ("footer,f", po::bool_switch(&show_footer),
        "Show information from the footer (e.g. object counts)")
        ("metadata,m", po::bool_switch(&show_metadata), "Show only metadata")
        ("proto,p", po::bool_switch(&dump_proto), "Dump .proto files")
        ("input", po::value(&input_files),
        "input file names (runs once per specified file)")
    ;
    ...
    if (show_footer || dump_all) {
        FATAL_ASSERT((event_count == 1 && event_index == 0),
                     "Specifying event count or index is incompatible with --footer and --dump-all");
    }
