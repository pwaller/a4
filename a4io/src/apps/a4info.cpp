...
int main(int argc, char ** argv) {
    ...
    commandline_options.add_options()
        ...
        ("event-index,i", po::value(&event_index)->default_value(0),
        "event to start dumping from (starts at 0)")
        ("count,c", po::value(&event_count)->default_value(1),
        "number to dump'")
        ("input", po::value(&input_files),
        "input file names (runs once per specified file)")
        ("output", po::value(&output_file)->default_value(""),
        "output file name")
        ("var,v", po::value(&variables), "variables to dump (defaults to all)")
        ("collect-stats,S", po::value(&collect_stats),
    ...
    while (shared<a4::io::InputStream> stream = in.get_stream()) {
        ...
        foreach (const auto & header, all_metadata) {
            ...
            foreach (const auto & metadata, header) {
                if (outs) {
                    outs->metadata(*metadata->message());
