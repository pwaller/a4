...
namespace internal {
typedef boost::function<shared<Message>(google::protobuf::io::CodedInputStream *)>
    from_stream_func;
typedef boost::function<shared<Message>()>
    new_protoclass_func;
...
classreg map_class(std::string name, uint32_t class_id = 0,
                   classreg = classreg(), bool warn = false);
...
template<typename ProtoClass>
internal::classreg RegisterClass<ProtoClass>::registration =
    internal::reg_protoclass<ProtoClass>();
