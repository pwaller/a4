...
namespace a4 {
namespace io {
typedef boost::variant<int32_t, int64_t, uint32_t, uint64_t, double, float,
                       bool, std::string> FieldContentVariant;

struct variant_adder :
    public boost::static_visitor<FieldContentVariant> {
    template<typename T>
    FieldContentVariant operator()(const T & i, const T & j) const {
        return i +
               j;
    }
...
struct variant_multiplier :
    public boost::static_visitor<FieldContentVariant> {
    FieldContentVariant operator()(const std::string & i,
                                   const std::string & j) const
                                                         { return i + j; }
    FieldContentVariant operator()(const bool & i,
                                   const bool & j) const { return i && j; }
    template<typename T>
    FieldContentVariant operator()(const T & i, const T & j) const {
        return i *
               j;
...
struct variant_typeid :
    public boost::static_visitor<const std::type_info &> {
...
struct visit_setter :
    ...
    visit_setter(Message * m, const FieldDescriptor * f) :
        m(m),
        f(f),
        r(m->GetReflection()) {}
    Message               * m;
...
struct visit_adder :
    public boost::static_visitor<void> {
    visit_adder(Message * m, const FieldDescriptor * f) :
        m(m),
        f(f),
        r(m->GetReflection()) {}
    Message               * m;
    ...
    void operator()(uint32_t i) const {
        if (f->cpp_type() == FieldDescriptor::CPPTYPE_ENUM) {
            const EnumValueDescriptor * evd =
                f->default_value_enum()->type()->FindValueByNumber(i);
            r->AddEnum(m, f, evd);
        } else {
            r->AddUInt32(m, f, i);
        }
...
public:
    ...
    FieldContent(const Message & m, const FieldDescriptor * f, int i) {
        ...
        case FieldDescriptor::CPPTYPE_INT32:
            content = r->GetRepeatedInt32(m, f, i);
            break;

        case FieldDescriptor::CPPTYPE_INT64:
            content = r->GetRepeatedInt64(m, f, i);
            break;

        case FieldDescriptor::CPPTYPE_UINT32:
            content = r->GetRepeatedUInt32(m, f, i);
            break;

        case FieldDescriptor::CPPTYPE_UINT64:
            content = r->GetRepeatedUInt64(m, f, i);
            break;

        case FieldDescriptor::CPPTYPE_DOUBLE:
            content = r->GetRepeatedDouble(m, f, i);
            break;

        case FieldDescriptor::CPPTYPE_FLOAT:
            content = r->GetRepeatedFloat(m, f, i);
            break;

        case FieldDescriptor::CPPTYPE_BOOL:
            content = r->GetRepeatedBool(m, f, i);
            break;

        case FieldDescriptor::CPPTYPE_ENUM:
            content = (uint32_t) r->GetRepeatedEnum(m, f, i)->number();
            break;

        case FieldDescriptor::CPPTYPE_STRING:
            content = r->GetRepeatedString(m, f, i);
            break;
    ...
    FieldContent(const Message & m, const FieldDescriptor * f) {
        ...
        switch (f->cpp_type()) {
        case FieldDescriptor::CPPTYPE_INT32:
            content = r->GetInt32(m, f);
            break;

        case FieldDescriptor::CPPTYPE_INT64:
            content = r->GetInt64(m, f);
            break;

        case FieldDescriptor::CPPTYPE_UINT32:
            content = r->GetUInt32(m, f);
            break;

        case FieldDescriptor::CPPTYPE_UINT64:
            content = r->GetUInt64(m, f);
            break;

        case FieldDescriptor::CPPTYPE_DOUBLE:
            content = r->GetDouble(m, f);
            break;

        case FieldDescriptor::CPPTYPE_FLOAT:
            content = r->GetFloat(m, f);
            break;

        case FieldDescriptor::CPPTYPE_BOOL:
            content = r->GetBool(m, f);
            break;

        case FieldDescriptor::CPPTYPE_ENUM:
            content = (uint32_t) r->GetEnum(m, f)->number();
            break;

        case FieldDescriptor::CPPTYPE_STRING:
            content = r->GetString(m, f);
            break;
...
public:
    ...
    ConstDynamicField(const Message & m, const FieldDescriptor * f) :
        _m(m),
        _f(f),
        _r(m.GetReflection()) {}
