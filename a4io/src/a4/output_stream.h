...
public:
    ...
    OutputStream & set_forward_metadata() {
        assert(!_opened);
        _metadata_refers_forward = true;
        return *this;
    }

    /// String representation of this stream for user output
    std::string str() {
        return std::string("OutputStream(\"") + _output_name +
               "\", \"" + _description + "\")";
    }
private:
    ...
    void write_protoclass(uint32_t                             class_id,
                          const google::protobuf::Descriptor * d);
    uint32_t find_class_id(const google::protobuf::Descriptor * d,
                           bool                                 metadata);

    std::map<const std::string, uint32_t> _class_id;
    std::vector<int>                      _class_id_counts;
    bool have_written_classid(const uint32_t & classid) {
        return
            _written_classids.find(classid) != _written_classids.end();
    }
    void set_written_classid(const uint32_t & classid)  {
        _written_classids.
        insert(classid);
    }
};
