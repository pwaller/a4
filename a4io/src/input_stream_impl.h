...
public:
    ...
    bool skip_to_next_metadata() {
        return seek_to(_current_header_index, _current_metadata_index + 1,
                       true);
    }
    ...
    shared<a4::io::A4Message >>> & all_metadata() {
        if (_metadata_per_header.size() == 0) {
            if (_started) {
                FATAL(
                    "Coding Bug: all_metadata first called after reading started!");
            }
            startup(true);
        }
    ...
    const std::vector<StreamFooter> & footers() {
        if (_footers.size() == 0) {
            if (_started) {
                FATAL(
                    "Coding Bug: footers() first called after reading started!");
            }
            startup(true);
        }
        return _footers;
...
inline
bool InputStreamImpl::try_read(Message                            & msg,
    ...
    if (_current_class_pool->check_match(class_id, d)) {
        ...
        auto _message = _current_class_pool->parse_message(class_id,
                                                           _coded_in,
                                                           size);
...
shared<A4Message> InputStreamImpl::bare_message() {
    ...
    if (_hint_copy) {
        shared<A4Message> umsg(new A4Message(class_id,
                                             size,
                                             _coded_in,
                                             _current_class_pool));
        ...
        auto _message = _current_class_pool->parse_message(class_id,
                                                           _coded_in,
                                                           size);
        return shared<A4Message>(new A4Message(class_id, _message,
                                               _current_class_pool));
    }
}
    ...
    if (_do_reset_metadata) {
        ...
        if (_metadata_per_header.size() > _current_header_index) {
            auto & header_metadata =
                _metadata_per_header[_current_header_index];
            if (static_cast<int32_t>(header_metadata.size()) >
                _current_metadata_index) {
