...
OpenFile::OpenFile(const char * name, int oflag, bool do_mmap) {
    ...
    struct stat buffer;
    static_assert(sizeof(buffer.st_size) >= sizeof(size_t),
                  "OS Size information is 32 bit!");
...
UnixFileMMap::UnixFileMMap(std::string name) :
    _name(name),
    _size(0),
    _position(0),
    _mmap(0),
...
UnixFilePartMMap::UnixFilePartMMap(std::string name) :
    UnixFileMMap(name),
    _mmap_offset(0),
    _mmap_blocksize(1 << 28) {}
...
bool UnixFilePartMMap::Next(const void ** data, int * size) {
    ...
    if (!_open && !open()) { return false; }
    if (_size == _position) { return false; }
    if ((_position < _mmap_offset) ||
        (_position >= _mmap_offset + _mmap_blocksize)) {
...
UnixStream::UnixStream(int file_descriptor, int block_size) {
    _impl.reset(new google::protobuf::io::FileInputStream(file_descriptor,
                                                          block_size));
}

bool UnixStream::Next(const void ** data, int * size) { return _impl->Next(data,
                                                                           size);
}
void UnixStream::BackUp(int count) {
    return _impl->BackUp(count);
}
bool UnixStream::Skip(int count) {
    return _impl->Skip(count);
}
google::protobuf::int64 UnixStream::ByteCount() const                    {
...
UnixFile::UnixFile(std::string filename, int block_size) {
    ...
    _impl.reset(new google::protobuf::io::FileInputStream(_file->no,
                                                          block_size));
}
...
bool RemoteFile::Next(const void ** data, int * size) { return _adaptor->Next(
                                                            data,
                                                            size); }
void RemoteFile::BackUp(int count) {
    return _adaptor->BackUp(count);
}
bool RemoteFile::Skip(int count) {
    return _adaptor->Skip(count);
}
bool RemoteFile::Seek(size_t position) {
    _adaptor.reset();
    bool res = _file->Seek(position);
    new_adaptor();
    return res;
}
bool RemoteFile::SeekBack(size_t position) {
    _adaptor.reset();
    bool res = _file->SeekBack(position);
    new_adaptor();
    return res;
}
size_t RemoteFile::Tell() const                         {
    _adaptor.reset();
    size_t res = _file->Tell();
    _byte_count = res;
    new_adaptor(true);
    return res;
}
google::protobuf::int64 RemoteFile::ByteCount() const                    {
    return _byte_count + _adaptor->ByteCount();
}
void RemoteFile::new_adaptor(bool tell) const {
    if (!tell) { _byte_count = Tell(); }
    _adaptor.reset(new google::protobuf::io::CopyingInputStreamAdaptor(
                       _file.get(),
                       _block_size));
}
...
UNIQUE<ZeroCopyStreamResource> resource_from_url(std::string url) {
    ...
    if (S_ISFIFO(buffer.st_mode) || !try_mmap) {
        return UNIQUE<UnixFile>(new UnixFile(
                                    url));
    }
