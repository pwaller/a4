...
class rfio_filesystem_calls :
    public filesystem_calls {
public:
    ...
    virtual int open(const char * p1, int p2, mode_t p3)     {
        return _open(p1,
                     p2,
                     p3);
    }
    virtual size_t read(int p1, void * p2, size_t p3)        {
        return _read(p1,
                     p2,
                     p3);
    }
    virtual off_t lseek(int p1, off_t p2, int p3)            {
        return _lseek(p1,
                      p2,
                      p3);
    }
    virtual size_t write(int p1, const void * p2, size_t p3) {
        return _write(p1,
                      p2,
                      p3);
    }
    virtual int close(int p1)                                {
        return _close(p1);
    }
    void * _library;
private:
...
public:
    ...
    virtual off_t lseek(int p1, off_t p2, int p3) {
        if (p3 == SEEK_END) {
            return _lseek(p1, _lseek(p1,
                                     p2,
                                     p3), SEEK_SET);
        } else {
            return _lseek(p1, p2, p3);
        }
    }
    virtual size_t write(int p1, const void * p2, size_t p3) {
        return _write(p1,
...
private:
    ...
    hdfsFile (* _hdfsOpenFile)(hdfsFS, const char *, int, int, short,
                               int32_t);
