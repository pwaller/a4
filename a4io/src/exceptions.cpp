...
BackTraceException::BackTraceException() {
    ...
    cmd1 <<
        "gdb -n --batch --eval-command='set complaints 0' --eval-command='attach "
         << getpid() << "' --eval-command='thread apply all bt' 2>&1";
    cmd2 <<
        "gdb -n --batch --eval-command='set complaints 0' --eval-command='attach "
         << getpid() << "' --eval-command='thread apply all bt full' 2>&1";
    char line[256];
    if (!(fpipe =
              (FILE *) popen(cmd1.str().c_str(), "r"))) { gdb_worked = false;
    } else {
    ...
    if (!(fpipe =
              (FILE *) popen(cmd2.str().c_str(), "r"))) { gdb_worked = false;
    } else {
        while (fgets(line, sizeof line, fpipe)) {
            res2 << line;
        }
...
void Fatal::handle_exception(std::string reason) {
    ...
    try {
        ...
        out << sstr.str() << std::endl << "FULL BACKTRACE:" << std::endl <<
            _full_backtrace << std::endl;
        sstr << "\nNotice: Error has been written to " << dirname.string() <<
            std::endl;
    } catch (...) {}
    ;
    sstr <<
        "If this is a bug in A4, please submit an issue at https://github.com/JohannesEbke/a4/issues"
         << std::endl;
    _what = sstr.str();
}
...
void Fatal::segfault_handler(int i) {
    if (i == SIGALRM) {
        std::cerr <<
            "GDB backtrace timed out. Please run the debugger manually to obtain a backtrace."
                  << std::endl;
        std::terminate();
    }
    std::cerr << "A4 segfault handler called..." << std::endl;
    {
        ...
        if (segfault_handled) {
            std::cerr <<
                "double segfault or segfault in exception handler - sorry!" <<
                std::endl;
            std::terminate();
        }
