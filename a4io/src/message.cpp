...
A4Message::A4Message(
            uint32_t                                            class_id,
                     size_t                                              size,
                     weak_shared<google::protobuf::io::CodedInputStream> coded_in,
                     shared<ProtoClassPool>                              pool) :
    _class_id(class_id),
    ...
    _bytes(),
    _message(),
    _instream_read(false) {
    assert_valid();
}

/// Constructs an read A4Message connected to a ProtoClassPool
A4Message::A4Message(uint32_t class_id, shared<Message> msg,
                     shared<ProtoClassPool> pool) :
    _class_id(class_id),
...
A4Message::A4Message(shared<google::protobuf::Message> msg, bool metadata) :
...
/// Construct an A4Message from a compiled-in protobuf Message
A4Message::A4Message(const google::protobuf::Message & msg, bool metadata) :
    _class_id(metadata ? NO_CLASS_ID_METADATA : NO_CLASS_ID),
    _descriptor(msg.GetDescriptor()),
    _pool(),
    _size(0),
...
void A4Message::self_version_check() const {
    ...
    if (d1->full_name() != d2->full_name()) {
        FATAL("Compiled-in (",
              d1->full_name(), ") and on-disk (",
              d2->full_name(), ") typenames of an object do not agree!");
    }
    ...
    if (mymajor != dmajor) {
        FATAL("Major versions of compiled-in and on-disk '",
              d1->full_name(), "' objects do not agree:", mymajor, " != ",
              dmajor);
    } else if (myminor != dminor) {
        WARNING("Minor versions of compiled-in and on-disk '",
                d1->full_name(), "' objects do not agree:", mymajor, " != ",
                dmajor);
    }
}
...
void A4Message::version_check(const A4Message & m2) const {
    ...
    if (descriptor()->full_name() != m2.descriptor()->full_name()) {
        FATAL("Typenames of objects to merge do not agree: ",
              descriptor()->full_name(), " != ",
              m2.descriptor()->full_name());
    }
    ...
    if (mymajor != dmajor) {
        FATAL("Major versions of objects to merge do not agree:",
              mymajor,
              " != ",
              dmajor);
    } else if (myminor != dminor) {
...
A4Message & A4Message::operator+=(const A4Message & m2_) {
    ...
    for (int i = 0; i < d->field_count(); i++) {
        MetadataMergeOptions merge_opts = d->field(i)->options().GetExtension(
            merge);
        ...
        case MERGE_BLOCK_IF_DIFFERENT:
            if (!(f1 == f2)) {
                ...
                throw a4::Fatal(
                          "Trying to merge metadata objects with different entries in ",
                          f1.name(),
                          ":",
                          f1.value().str(),
                          " != ",
        ...
        default:
            throw a4::Fatal("Unknown merge strategy: ",
                            merge_opts,
                            ". Recompilation should fix it.");
        }
    }
    assert_valid();
    return *this;
...
bool A4Message::check_key_mergable(const A4Message   & rhs,
    ...
    case MERGE_APPEND:
    case MERGE_DROP:
        FATAL("Merge is keyed on ",
              lhs_field->name(), " which has merge == ",
              MetadataMergeOptions_Name(merge_opt));
    }
    ...
    if (lhs_field->repeated()) {
        if (merge_opt == MERGE_UNION) {
            ...
            return lhs_field->value(lhs_field->size() - 1) ==
                   rhs_field->value(0);
        } else {
            FATAL(
                "Tried to merge keying on repeated field with non MERGE_UNION: ",
                lhs_field->name(),
                " has merge == ",
                MetadataMergeOptions_Name(merge_opt));
        }
...
std::string A4Message::assert_field_is_single_value(
    const std::string & field_name) const {
    assert(descriptor() == message()->GetDescriptor());
    assert_valid();
    const FieldDescriptor * fd = descriptor()->FindFieldByName(field_name);
    if (!fd) {
        ...
        FATAL(classname,
              " has no member ",
              field_name,
              " necessary for metadata merging or splitting!");
    }
    if (fd->is_repeated() &&
        ((message()->GetReflection()->FieldSize(*message(), fd)) > 1)) {
        FATAL(
            fd->full_name(), " has already multiple ", field_name,
            " entries - cannot achieve desired granularity!");
    }
...
shared<ConstDynamicField> A4Message::dynamic_field(
    const std::string & field_name) const {
    const FieldDescriptor * fd = descriptor()->FindFieldByName(field_name);

    FATAL_ASSERT(fd, "Field '", field_name, "' doesn't exist on ",
                 descriptor()->full_name());
