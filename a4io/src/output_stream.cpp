...
OutputStream::OutputStream(
    shared<google::protobuf::io::ZeroCopyOutputStream> out,
...
bool OutputStream::open() {
    ...
    if (_fileno == -1) {
        int fd = ::open(
            _output_name.c_str(), O_WRONLY | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP |
            S_IROTH);
        if (fd < 0) {
            std::cerr << "ERROR - A4IO:OutputStream - Could not open '" <<
                _output_name \
...
bool OutputStream::close() {
    ...
    if (_file_out && !_file_out->Close()) {
        std::cerr << "ERROR - A4IO:OutputStream - Error on closing: " <<
            _file_out->GetErrno() << std::endl;
        return false;
...
uint32_t OutputStream::find_class_id(const google::protobuf::Descriptor * d,
                                     bool                                 metadata)
{
    ...
    if (i != _class_id.end()) {
        if (metadata != (i->second % 2 == 1)) {
            FATAL("Sorry, you can use the same Message class (",
                  d->full_name(),
                  ") either for metadata or for content, but not both.");
        }
        return i->second;
    }
    uint32_t class_id = 0;
    ...
    if (_next_metadata_class_id == 101) {
        _next_metadata_class_id += 100; // skip the 100's, since the a4 messages are there.
    }
    if (_next_class_id == 100) {
        _next_class_id += 100; // skip the 100's, since the a4 messages are there.
    }
...
OutputStream & OutputStream::set_compression(CompressionType t, int level) {
    ...
    if ((level < 1) || (level > 9)) {
        a4::Fatal(
            "Only compression levels between 1 and 9 are meaningful.");
    }
    _compression_level = level;
#ifndef HAVE_SNAPPY
    if (t == SNAPPY) {
        a4::Fatal(
            "Compression with 'snappy' is specified, but the library is not compiled in!");
    }
#endif
    return *this;
}
...
bool OutputStream::start_compression() {
    ...
    if (_compression_type == SNAPPY) {
        cs_header.set_compression(
            StartCompressedSection_Compression_SNAPPY);
    } else if (_compression_type == ZLIB) {
...
std::vector<const google::protobuf::FileDescriptor *> get_file_descriptors(
    ...
    for (int i = 0; i < file_descriptor->dependency_count(); i++) {
        const auto tmp = get_file_descriptors(seen_fds,
...
std::vector<const google::protobuf::FieldDescriptor *> get_extension_descriptors(
    ...
    for (int i = 0; i < d->field_count(); i++) {
        ...
        if (field.cpp_type() ==
            ...
            const auto tmp = get_extension_descriptors(seen_descriptors,
                                                       field.message_type());
...
void OutputStream::write_protoclass(
    uint32_t                             class_id,
    const google::protobuf::Descriptor * d) {
    ProtoClass a4proto;
    ...
    std::set<const google::protobuf::FileDescriptor *> seen_fds;
    auto                                               file_descriptors =
        get_file_descriptors(seen_fds, d->file());

    // Fetch extensions on all subclasses of this descriptor
    std::set<const google::protobuf::Descriptor *> seen_descriptors;
    auto                                           extensions =
        get_extension_descriptors(seen_descriptors, d);
...
bool OutputStream::write(uint32_t                          class_id,
