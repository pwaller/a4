...
std::vector<std::string> get_list_of_leaves() {
    ...
    foreach (std::string s, leaf_names) {
        res.push_back(s);
    }
    return res;
}
...
template<typename SOURCE_TYPE, typename PROTOBUF_TYPE>
void call_adder(Message                                             * message,
                TBranchElement                                      * br,
                const typename Setter<PROTOBUF_TYPE>::ProtobufAdder & adder,
                const FieldDescriptor                               * field) {
    const vector<SOURCE_TYPE> & values =
...
#define DEFINE_SETTERS(T, ProtobufTypename)                                   \
    template<> Setter<T>::ProtobufSetter reflection_setter<T>(                \
        ...
        return bind(&::google::protobuf::Reflection::Set ## ProtobufTypename, \
                    reflection,                                               \
                    _1,                                                       \
                    field,                                                    \
                    _2);                                                      \
    ...
    template<> Setter<T>::ProtobufGetter reflection_getter<T>(                \
        const Reflection * reflection, const FieldDescriptor * field) {       \
        return bind(&::google::protobuf::Reflection::Get ## ProtobufTypename, \
                    reflection,                                               \
                    _1,                                                       \
                    field);                                                   \
    }                                                                         \
    ...
    template<> Setter<T>::ProtobufAdder reflection_adder<T>(                  \
        const Reflection * reflection, const FieldDescriptor * field) {       \
        return bind(&::google::protobuf::Reflection::Add ## ProtobufTypename, \
                    reflection,                                               \
                    _1,                                                       \
                    field,                                                    \
                    _2);                                                      \
...
template<typename SOURCE_TYPE, typename PROTOBUF_TYPE>
Copier make_field_setter(TBranch               * branch,
                         TLeaf                 * leaf,
    ...
    if (f->is_repeated()) {
        typename Setter<PROTOBUF_TYPE>::ProtobufSetter protobuf_rsetter =
            reflection_adder<PROTOBUF_TYPE>(r, f);
        typename Setter<PROTOBUF_TYPE>::RepeatedSetterCaller adder =
            call_adder<SOURCE_TYPE, PROTOBUF_TYPE>;
        return bind(adder, _1, (TBranchElement *) branch, protobuf_rsetter, f);
    }
    typename Setter<PROTOBUF_TYPE>::ProtobufSetter protobuf_setter =
        reflection_setter<PROTOBUF_TYPE>(r, f);
    typename Setter<PROTOBUF_TYPE>::ProtobufGetter protobuf_getter =
        reflection_getter<PROTOBUF_TYPE>(r, f);
    typename Setter<PROTOBUF_TYPE>::SetterCaller setter_caller =
        call_setter<SOURCE_TYPE, PROTOBUF_TYPE>;
    void * pointer = leaf->GetValuePointer();
    return bind(setter_caller, protobuf_setter, protobuf_getter, _1, pointer);
}

Copier make_copier_from_leaf(TBranch               * branch,
                             TLeaf                 * leaf,
                             const FieldDescriptor * field,
                             const Reflection      * refl) {
#define TRY_MATCH(root_source_type, plain_source_type, dest_type)       \
    if (leaf_type == # plain_source_type || leaf_type ==                \
        "vector<" # plain_source_type ">") {                            \
        return make_field_setter<plain_source_type, dest_type>(branch,  \
                                                               leaf,    \
                                                               field,   \
                                                               refl); } \
    else if (leaf_type == # root_source_type || leaf_type ==            \
             "vector<" # root_source_type ">") {                        \
        return make_field_setter<root_source_type, dest_type>(branch,   \
                                                              leaf,     \
                                                              field,    \
                                                              refl); }
...
#define FAILURE(protobuf_typename)                                                    \
    FATAL(str_cat("a4root doesn't know how to convert the branch ",                   \
                  ...
                  "protobuf=", protobuf_typename,                                     \
                  ". The .proto is probably wrong. "                                  \
...
#include "apps/root2a4/convertable_types.cc"
    ...
    default:
        FATAL("Unknown field type in make_copier_from_branch ",
              field->cpp_type());
    }
...
void run_copiers(Message               * message,
...
void submessage_factory(Message                 * parent,
...
#define TRY_MATCH(ctype)                                             \
    ...
    FATAL("a4root doesn't know how to count the ",
          ...
          "or fix it yourself at " __FILE__ ":",
          __LINE__,
          ".");
...
#define BIND(source_type, destination_type)                              \
    bind(submessage_setter<source_type, destination_type>, _1, _2,       \
         branch_element, reflection_setter<destination_type>(reflection, \
                                                             field), field)

#define VBIND(source_type, destination_type)                            \
    bind(submessage_adder<source_type, destination_type>, _1, _2,       \
         branch_element, reflection_adder<destination_type>(reflection, \
                                                            field), field)

    // !Note!: `source_type` is expanded as a string!!
#define TRY_MATCH(root_source_type, plain_source_type,                        \
                  protobuf_destination_type)                                  \
...
#define FAILURE(protobuf_typename)                                                    \
    FATAL(str_cat("a4root doesn't know how to convert the branch ",                   \
                  ...
                  field->full_name(), " with types ROOT=", branch_typename,           \
                  " and "                                                             \
                  "protobuf=", protobuf_typename,                                     \
                  ". The .proto is probably wrong. "                                  \
                  "Alternatively, if the types are compatible and conversion should " \
                                        ...
                                        MessageFactory        * dynamic_factory)
{
    ...
    foreach (const FieldDescriptor * field, get_fields(desc)) {
        if (!field->options().HasExtension(root_branch)) {
            const std::string warning = str_cat(
                field->full_name(),
        ...
        const std::string leafname = prefix + field->options().GetExtension(
            root_branch);

        TBranchElement * br =
            dynamic_cast<TBranchElement *>(tree->GetBranch(leafname.c_str()));
        if (!br) {
    ...
    Copier copier = bind(submessage_factory,
                         _1,
                         parent_refl,
                         parent_field,
                         submessage_setters,
...
Copier make_submessage_factory(TTree                 * tree,
    ...
    foreach (auto field, get_fields(desc)) {
        ...
        if (specifies_branch ||
            ...
            if (!leaf) {
                std::cerr <<
                    "Branch specified in protobuf file but not in TTree: " <<
                    leafname << std::endl;
                continue;
            }
            leaf_names.insert(leafname);
            ...
            const std::string this_prefix = prefix +
                                            field->options().GetExtension(
                root_prefix);
            copiers.push_back(make_repeated_submessage_factory(tree, refl,
                                                               field,
                                                               this_prefix,
                                                               dynamic_factory));
        }
...
RootToMessageFactory make_message_factory(TTree             * tree,
    ...
    foreach (auto field, get_fields(desc)) {
        ...
        if (specifies_branch ||
            ...
            if (specifies_prefix) {
                copiers.push_back(make_submessage_factory(tree, refl, field,
                                                          leafname,
                                                          dynamic_factory));
                continue;
            }
