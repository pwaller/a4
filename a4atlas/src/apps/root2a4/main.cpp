...
typedef std::vector<shared<Message> >
                                                      MessageBuffer;
typedef boost::function<void (shared<a4::io::OutputStream>,
                              const MessageBuffer &)> MetadataCallback;
...
public:
    ...
    virtual bool need_new_metadata(shared<Message> event) {
        ...
        if (_field_mc_channel && _refl->HasField(*event, _field_mc_channel)) {
            const uint32_t mc_channel = _refl->GetUInt32(*event,
                                                         _field_mc_channel);
    ...
    virtual void compute_info(const MessageBuffer & buffer) {
        ...
        p.set_walltime((chrono::steady_clock::now() -
                        _start_wallclock).count());
        ...
        if (buffer.size() >= 1) {
            ...
            if (_field_mc_channel &&
                _refl->HasField(*buffer[0], _field_mc_channel)) {
                _metadata.add_mc_channel(_refl->GetUInt32(*buffer[0],
                                                          _field_mc_channel));
            }
        }
    }
...
public:
    ...
    virtual void reset_counters() {
        ...
        TObjString * grl_string =
            dynamic_cast<TObjString *>(input_file->Get(grl_path.c_str()));
...
public:
    ...
    EventFactoryBuilder(TTree                & t,
                        const Descriptor     * d,
                        RootToMessageFactory * f,
                        MessageFactory       * dynamic_factory,
                        FlushCallback          flush_callback) :
        _tree(t),
        _descriptor(d),
        _factory(f),
        _dynamic_factory(dynamic_factory),
    ...
    Bool_t Notify() {
        assert(_descriptor);
        std::cout << "Processing new file: " <<
            _tree.GetCurrentFile()->GetName() << std::endl;
        if (!_brand_new) {
            _flush_callback();
        }
        _brand_new = false;
        //std::cout << "Notify start" << std::endl;
        (*_factory) = make_message_factory(&_tree,
                                           _descriptor,
                                           "",
                                           _dynamic_factory);
...
public:
    ...
    ~BufferingStreamWriter() {
        std::cout << "Destructor flush " << std::endl;
...
/// Event class.
void copy_chain(TChain                     & tree,
                shared<a4::io::OutputStream> stream,
                MessageFactory             * dynamic_factory,
                const Descriptor           * message_descriptor,
                Long64_t                     entries=-1,
                const uint32_t               metadata_frequency=100000,
    ...
    // Event class.
    EventFactoryBuilder builder(tree,
                                message_descriptor,
                                &event_factory,
                                dynamic_factory,
                                bind(&BufferingStreamWriter::flush, boost::ref(
                                         buffered_stream)));
    ...
    std::cout << "Copied " << entries << " entries (" << total_bytes_read <<
        ")" << std::endl;
}
...
int main(int argc, char ** argv) {
    ...
    assert(
        gROOT->GetVersionCode() == ROOT_VERSION_CODE &&
        "Check build ROOT version check against dynamic library");
    ...
    commandline_options.add_options()
        ...
        ("tree-type,T",
        po::value<std::string>(&tree_type)->default_value("test"),
        "which event factory to use (SMWZ, PHOTON, test or .proto file with Event message)")
        ("input,i", po::value<std::vector<std::string> >(&input_files),
        "input file names")
        ("output,o",
        po::value<std::string>(&output_file)->default_value("test_io.a4"),
        "output file name")
        ("number,n", po::value<Long64_t>(&event_count),
        "number of events to process (-1=all available)")
        ("offset,O", po::value<Long64_t>(&initial_offset),
        "initial offset (0=first event)")
        ("compression-type,C",
        po::value(&compression_type)->default_value(default_compression),
        "compression level '[TYPE] [LEVEL]'")
        ("metadata-frequency,m",
        po::value(&metadata_frequency)->default_value(metadata_frequency),
        "Metadata frequency [N] (1 per N)")
    ;
    ...
    foreach (const std::string & input_file, input_files) {
        if (string_endswith(input_file, ".txt")) {
            ...
            foreach (const std::string & x, fileList) {
                std::cout << "--Adding sub input file: '" << x << "'" <<
                    std::endl;
                input.Add(x.c_str());
            }
        } else {
    ...
    DescriptorPool source_pool(&source_tree_db,
                               source_tree_db.GetValidationErrorCollector());
    ...
    DescriptorPool pool(&merged_pool,
                        source_tree_db.GetValidationErrorCollector());
