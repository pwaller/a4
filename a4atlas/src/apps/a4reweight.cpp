...
class A4ReweightProcessor :
    public ResultsProcessor<A4ReweightProcessor, EventMetaData, H1, H2, H3,
                            Cutflow> {
public:
    ...
    void process(std::string name, shared<Storable> s) {
        if (S.get_slow<Storable>(name)) {
            FATAL(
                "Encountered the same histogram twice! Merge files before reweighting them!");
        } else {
            *s *= weight;
            S.set_slow(name, s);
        }
...
class A4ReweightConfiguration :
    public ConfigurationOf<A4ReweightProcessor> {
public:
    ...
    void add_options(po::options_description_easy_init opt) {
        ...
        opt("xs,x", po::value(
                &xs_file),
            "Cross-Section file (Lines of the form <MC-ID> <XS [pb]>)");
        opt("force,F", po::bool_switch(&force)->default_value(
                false), "Reweight even if metadata().simulation() is not set");
        opt("run-number", po::bool_switch(&use_run_number)->default_value(
                false), "Use run number instead of mc channel.");
    }
