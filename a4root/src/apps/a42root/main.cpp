...
public:
    ...
    MessageTreeNode(const FieldDescriptor * field,
...
#undef CLEAR
    ...
    shared<MessageTreeNode> add_child(const FieldDescriptor * field) {
        ...
        auto * treenode = new MessageTreeNode(field,
                                              _depth,
                                              prefix,
                                              _tree);
    ...
    void make_tree(std::unordered_set<const FieldDescriptor *> seen_fds,
        ...
        for (int i = 0; i < descriptor->field_count(); i++) {
            ...
            if (!seen_fds.insert(fd).second) {
                WARNING("Skipping recursive descriptor for the second time: ",
                        fd->full_name());
                continue;
            }
...
class a42rootProcessor :
    public a4::process::Processor {
public:
...
class A2RConfig :
    public a4::process::ConfigurationOf<a42rootProcessor> {
public:
