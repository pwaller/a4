...
class A2RProcessor :
    public ResultsProcessor<A2RProcessor, a4::io::NoProtoClass, H1, H2, H3,
                            Cutflow> {
public:
    ...
    void a4_hist_to_root(const std::string & name, H & h, TH1 * root_hist) {
        ...
        // Fallthrough
        case 3:
            a4_axis_to_root(h.axis(2), root_hist->GetZaxis());
            total_bins *= h.axis(2).bins() + 2;

        case 2:
            a4_axis_to_root(h.axis(1), root_hist->GetYaxis());
            total_bins *= h.axis(1).bins() + 2;

        case 1:
            a4_axis_to_root(h.axis(0), root_hist->GetXaxis());
            total_bins *= h.axis(0).bins() + 2;
            break;
        ...
        if (h.weights_squared()) {
            root_hist->Sumw2();
            for (uint32_t bin = 0; bin < total_bins; ++bin) {
                root_hist->SetBinError(bin,
                                       sqrt(*(h.weights_squared().get() +
                                              bin)));
            }
        }
    ...
    void process(const std::string & name, shared<H1> h) {
        a4_hist_to_root(name,
                        *h,
                        new TH1D());
    }
    void process(const std::string & name, shared<H2> h) {
        a4_hist_to_root(name,
                        *h,
                        new TH2D());
    }
    void process(const std::string & name, shared<H3> h) {
        a4_hist_to_root(name,
                        *h,
                        new TH3D());
    }
    ...
    void process(const std::string & name, shared<Cutflow> h) {
        ...
        TH1D                             * cf      = new TH1D(
            rpath.leaf().c_str(), h->title.c_str(),
            content.size(), 0, content.size());
        cf->Sumw2();
        int i = 0;
        for (vector<Cutflow::CutNameCount>::const_iterator it = content.begin();
             it != content.end();
             it++) {
            i++;
        ...
        cf->SetDirectory((TDirectory *) f->Get(rpath.parent_path().string().
                                               c_str()));
        cf->Write();
    }
...
class A2RConfig :
    public ConfigurationOf<A2RProcessor> {
public:
    ...
    void add_options(po::options_description_easy_init opt) {
        opt("weight,w", po::value(&weight)->default_value(
                1.0), "Multiplicative weight");
        opt("root-file,R", po::value(&filename), "ROOT output file");
        opt("compression-level,C",
            po::value(&compression_level)->default_value(1),
            "compression level");
    }
