...
struct ALorentzVector {
    ...
    ALorentzVector() :
        ...
        pz(0.0),
        E(0.0) {}
    ALorentzVector(const double & p1,
                   const double & p2,
                   const double & p3,
                   const double & E) :
        px(p1),
        ...
        pz(p3),
        E(E) {}
    ALorentzVector(const ALorentzVector & p) :
        px(p.px),
        py(p.py),
        pz(p.pz),
        E(p.E) {}
    ...
    double        theta() const {
        return px == 0.0 and py == 0.0 and pz ==
               0.0 ? 0.0 : atan2(pt(), py);
    }
...
static __attribute__((unused)) ALorentzVector operator*(
    const double         & sf,
    const ALorentzVector & rhs)
{
...
static __attribute__((unused)) std::ostream & operator<<(
    std::ostream         & os,
    const ALorentzVector & lv)
{
    return os << "ALV(" << lv.px << ", " << lv.py << ", " << lv.pz << "; " <<
        lv.E << ")";
}
