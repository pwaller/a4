...
namespace hist {
class H1 :
    public a4::store::StorableAs<H1, pb::H1> {
public:
    ...
    void constructor(const uint32_t & bins,
                     const double   & min,
                     const double   & max,
                     const char     * label="");
    ...
    void constructor(const std::initializer_list<double> & bins,
                     const char                          * label="") {
    ...
    const shared_array<double> weights_squared() const {
        return _weights_squared;
    } //TODO: only for copyin into TH1D
...
class H2 :
    public a4::store::StorableAs<H2, pb::H2> {
public:
...
private:
    ...
    void ensure_weights() {
        const uint32_t total_bins =
            (_x_axis->bins() + 2) * (_y_axis->bins() + 2);
...
class H3 :
    public a4::store::StorableAs<H3, pb::H3> {
public:
    ...
    void fill(const double & x,
        ...
        if (_weights_squared) {
            *(_weights_squared.get() + binx + skip_x *
              (biny + skip_y * binz)) += weight * weight;
        } else if (weight != 1.0) {
            ensure_weights();
            *(_weights_squared.get() + binx + skip_x *
              (biny + skip_y * binz)) += weight * weight;
...
private:
    ...
    void ensure_weights() {
        const uint32_t total_bins =
            (_x_axis->bins() +
             2) * (_y_axis->bins() + 2) * (_z_axis->bins() + 2);
