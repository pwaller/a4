...
namespace hist {
class Cutflow :
    public a4::store::StorableAs<Cutflow, pb::Cutflow> {
public:
...
public:
        ...
        CutNameCount(std::string n, double c, double w2) :
            name(n),
            count(c),
            weights_squared(w2) {}
    };
    ...
    virtual void constructor(const char * _title) {
        title = _title;
        constructor();
    }
    ...
    template<typename ... Args>
    inline void passed(const Args & ... args) {
        fillw(_current_weight,
              args ...);
    }

    template<typename ... Args>
    inline void passedw(const double & w, const Args & ... args) {
        fillw(w,
              args ...);
    }
