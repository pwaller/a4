...
void H3::constructor(const uint32_t & bins,
                     const double   & min,
                     const double   & max,
                     const char     * label) {
...
double H3::integral() const {
    ...
    for (uint32_t binx = 1, binsx = binx + _x_axis->bins();
         binsx > binx;
         ++binx) {
        for (uint32_t biny = 1, binys = biny + _y_axis->bins();
             binys > biny;
             ++biny) {
            for (uint32_t binz = 1, binzs = binz + _z_axis->bins();
                 binzs > binz;
                 ++binz) {
                integral +=
                    *(_data.get() + binx + skip_x * (biny + skip_y * binz));
            }
        }
    }
            ...
            for (uint32_t binz = 1, binzs = binz + _z_axis->bins();
                ...
                out << "[" << setw(3) << binx << ", " << biny << ", " << binz <<
                    ...
                    << setiosflags(ios::fixed) << setprecision(3) <<
                        *(_data.get() + binx + skip_x *
                          (biny + skip_y * binz)) << endl;
            }
        }
    }
}
