...
struct _test_process_as<This, T, TArgs ...> {
    static bool process(This * that, const std::string & n,
                        shared<Storable> s) {
        ...
        if (t) {
            ...
            return _test_process_as<This, TArgs ...>::process(that, n, s);
        }
    }
};
...
struct _test_process_as<This> {
    static bool process(This * that, const std::string & n,
                        shared<Storable> s) { return false; }
};
...
public:
    ...
    enum MetadataBehavior {
        AUTO, MANUAL_FORWARD, MANUAL_BACKWARD, DROP
    };
    ...
    Processor() :
        ...
        skip_to_next_metadata(false),
        rerun_systematics_current(NULL),
        locked(false),
        metadata_behavior(AUTO) {}
    virtual ~Processor() {}
...
protected:
    ...
    void set_metadata_behavior(MetadataBehavior m) {
        assert(!locked);
        metadata_behavior = m;
    }
...
public:
    ...
    const ProtoMetaData & metadata() {
        if (!metadata_message) {
            FATAL("No metadata at this time!"); // TODO: Should not be fatal
        }
        const ProtoMetaData * meta = metadata_message->as<ProtoMetaData>();
        if (!meta) {
            FATAL("Unexpected Metadata type: ",
                  typeid(*metadata_message->message()), " (Expected: ",
                  typeid(ProtoMetaData), ")");
        }
        return *meta;
    }
...
public:
    ...
    void process_message(shared<const A4Message> msg) {
        ...
        if (next) {
            if (!_test_process_as<This, Args ...>::process((This *) this,
                                                           next_name, next)) {
