...
class CopyConfiguration :
    public a4::process::ConfigurationOf<CopyProcessor> {
public:
    ...
    void add_options(po::options_description_easy_init opt) {
        opt("metadata-only,M", po::bool_switch(
                &_metadata_only), "Only copy metadata");
        opt("start-index", po::value(
                &_start_index), "Start copy from a particular index");
    }
