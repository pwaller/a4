...
class BaseOutputAdaptor :
    public OutputAdaptor {
public:
    ...
    virtual void metadata(shared<const A4Message> m) {
        FATAL(
            "To write metadata manually, you have to change the metadata_behavior of the Processor!");
    }

    void write(shared<const A4Message> m) {
        if (!in_block) {
            FATAL(
                "Whoa?? Writing outside of a metadata block? How did you do this?");
        }
...
class ManualOutputAdaptor :
    public BaseOutputAdaptor {
public:

    ManualOutputAdaptor(Driver    * d,
                        Processor * p,
                        bool        forward_metadata,
                        A4Output  * out,
                        A4Output  * res) :
...
void SimpleCommandLineDriver::simple_thread(SimpleCommandLineDriver * self,
    ...
    case Processor::MANUAL_FORWARD:
        if (self->metakey != "") {
            FATAL("This program is not compatible with metadata merging!"); // forward if no merging
        }
    ...
    default:
        FATAL("Unknown metadata behaviour specified: ",
              p->get_metadata_behavior());
    }
    ...
    while (shared<InputStream> instream = self->in->get_stream()) {
        ...
        while (shared<A4Message> msg = instream->next_with_metadata()) {
            ...
            if (instream->new_metadata()) { // Start of new metadata block
                ...
                total_metadata_seen++;
#endif
                shared<const A4Message> c_new_metadata =
                    instream->current_metadata();
                ...
                if (c_new_metadata) {
                    ...
                    if (auto_metadata) {
                        ...
                        FATAL(
                            "You must not modify metadata in process_new_metadata if auto_metadata is off!");
                    }
                }
            }
...
try {
    ...
    popt.add_options()
        ...
        ("disable-gdb", po::bool_switch(&no_gdb),
        "switch of internal segfault handling")
        ...
        ("number,n", po::value<int>(&number)->default_value(-1),
        "maximum number of events to process (default: all)")
        ("per,p", po::value<string>(&metakey),
        "granularity of output by metadata key (e.g. period, run, lumiblock...). Default is input granularity.")
        ("split-per,s", po::value<string>(&split_metakey),
        "granularity of output by metadata key (e.g. period, run, lumiblock...). Default is input granularity.")
        ("config,c", po::value<string>(),
        (string("configuration file [default is '") + config_filename +
         "']").c_str());

    po::positional_options_description positional_options;
    positional_options.add("input", -1);

    po::options_description cfgopt(
        "Configuration: (section [config] in configuration file)");
    cfgopt.add_options()
        ("config.threads,t",
        po::value<int>(&n_threads)->default_value(hw_threads),
        "run N multi-threads [# of cores]");
    ...
    if ((2 > argc) || arguments.count("help") || !arguments.count("input")) {
        std::cout << "Usage: " << argv[0] << " [Options] input file(s)" <<
                std::endl;
    ...
    if (!config_file && explicit_config_file) {
        ...
        std::cout << "Using implicit config file '" << config_filename
                  << "'. Override this with -c 'other_configfile.ini'." <<
                std::endl;
    }
    ...
    VERBOSE("CPU time: ",
            total.cputime,
            " (",
            total.events / total.cputime.count(),
            "Hz)");

    const double megabytes = total.bytes / (1024. * 1024.);
    VERBOSE("Total data read ",
            megabytes,
            " (MB) Rate: ",
            megabytes / walltime.count(),
            " (MB/s)");
